...
import Pipeline.Internal.Core.PipeList (PipeList(..), takeP, dropP, AppendP(..))
import Pipeline.Internal.Core.UUID (UUID)
import Pipeline.Internal.Common.IFunctor (IFunctor7, IFix7(..))
import Pipeline.Internal.Common.IFunctor.Modular ((:+:)(..))
import Pipeline.Internal.Common.Nat (SNat(..), IsNat(..))
import Pipeline.Internal.Common.TypeList (Take, Drop, Length, (:++))
import Pipeline.Internal.Backend.ProcessNetwork (Network(..), taskExecuter)
...
initialNetwork :: forall inputsS inputsT inputsA.
...
instance BuildNetwork Swap where
  buildNetwork n Swap = do
    ...
    where
      ...
      swapOutput (PipeCons c1 (PipeCons c2 PipeNil)) = return $
...
instance BuildNetwork DropL where
  buildNetwork n DropL = do
    ...
    return $ Network (threads n) (inputs n) output
    where
      dropLOutput :: PipeList '[f, g] '[a, b] '[f a, g b]
        -> IO (PipeList '[g] '[b] '[g b])
      dropLOutput (PipeCons _ (PipeCons c2 PipeNil)) = return $
        PipeCons c2 PipeNil
...
instance BuildNetwork DropR where
  buildNetwork n DropR = do
    ...
    return $ Network (threads n) (inputs n) output
    where
      dropROutput :: PipeList '[f, g] '[a, b] '[f a, g b]
        -> IO (PipeList '[f] '[a] '[f a])
      dropROutput (PipeCons c1 (PipeCons _ PipeNil)) = return $
        PipeCons c1 PipeNil
...
beside n (Beside l r) = do
  ...
  where
    splitNetwork :: nbsLS ~ nbsLT
      => SNat nbsL
      -> IO (Network asS asT asA (Take nbsL bsS) (Take nbsL bsT) (Take nbsL bsA),
             Network asS asT asA (Drop nbsL bsS) (Drop nbsL bsT) (Drop nbsL bsA))
    splitNetwork nbs = return (Network (threads n) (inputs n) (takeP nbs (outputs n)),
