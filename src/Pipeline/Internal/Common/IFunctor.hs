...
icata2
  :: IFunctor2 iF => (forall a b . iF f a b -> f a b) -> IFix2 iF a b -> f a b 
icata2 alg (IIn2 x) = alg (imap2 (icata2 alg) x)

icata4
  :: IFunctor4 iF
  => (forall a b c d . iF f a b c d -> f a b c d) 
  -> IFix4 iF a b c d
  -> f a b c d
...
icataM7 :: (IFunctor7 iF, Monad m)
  => (forall a b c d e f g. iF f' a b c d e f g -> m (f' a b c d e f g)) -> IFix7 iF a b c d e f g -> m (f' a b c d e f g)
