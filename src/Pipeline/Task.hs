...
multiInputTask :: (DataStore' fs as,
                    DataStore g b, Task :<: iF, Eq (g b), Show (g b)) => (HList as -> b) -> g b -> IFix7 iF fs as (Apply fs as) '[g] '[b] '[g b] (Length fs)
multiInputTask f output = IIn7 (inj (Task (\sources sink -> do
  input <- (hSequence . fetch') sources
  save sink (f input)) output))
  
functionTask :: (DataStore f a, DataStore g b, Task :<: iF, Eq (g b), Show (g b)) => (a -> b) -> g b -> IFix7 iF '[f] '[a] '[f a] '[g] '[b] '[g b] ('Succ 'Zero)
