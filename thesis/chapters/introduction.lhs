...
%% \section{Motivation}
It is estimated that every day over 2.5 quintillion bytes of data is generated, by people around the world.
With such vast quantities of data, being able to process it is not becoming more important then ever before.
...
There are existing libraries that make use of the \ac{DAG} structure, to orchestrate tasks, such as the transformation of data or training a machine learning model: Spotify's Luigi~\cite{spotify_luigi} or Apache's Airflow.
...
Both Luigi and Airflow, apply techniques that describe how to compose tasks.
...
This it describes a method that can be used for safely transforming a circuit into a process network in a type-safe way, using principled recursion schemes.
With a working network in place, a it will describe how a network will be modified to capture errors and propagate them through the network.
